...
public class WeChatDecorator extends NevoDecoratorService {
	...
	@Override public void apply(final MutableStatusBarNotification evolving) {
		final MutableNotification notification = evolving.getNotification();
		...
		notification.color = PRIMARY_COLOR;		// Tint the small icon
		...
		MessagingStyle messaging = buildFromCarExtender(evolving.getKey(), notification, extras);
		if (messaging == null) messaging = buildFromArchive(evolving, notification, title, extras);
		if (messaging == null) return;

		// Add additional replies filled by us in the proxied procedure of direct-reply.
		final CharSequence[] inputs = extras.getCharSequenceArray(EXTRA_REMOTE_INPUT_HISTORY);
	...
	private @Nullable MessagingStyle buildFromCarExtender(final String key, final MutableNotification notification, final Bundle extras) {
		...
		if (parcelable_messages.length == 0) {
			messaging.addMessage(extras.getCharSequence(Notification.EXTRA_TEXT), 0, SENDER_PLACEHOLDER);
		} else {
			final List<MessagingStyle.Message> messages = new ArrayList<>();
			boolean has_named_sender = false;
			for (final Parcelable parcelable_message : parcelable_messages) {
		...
		if (on_read != null) notification.deleteIntent = on_read;						// Swipe to mark read
		...
		if (on_reply != null) {
			...
			if (remote_input != null) {
				final CharSequence[] input_history = extras.getCharSequenceArray(EXTRA_REMOTE_INPUT_HISTORY);
		...
		if (latest_timestamp != 0) notification.when = latest_timestamp;
		return messaging;
	}
	...
	@RequiresApi(KITKAT_WATCH) private @Nullable PendingIntent proxyDirectReply(
		...
		if (input_history != null) proxy_intent.putCharSequenceArrayListExtra(EXTRA_REMOTE_INPUT_HISTORY, new ArrayList<>(Arrays.asList(input_history)));
	...
	private final BroadcastReceiver mReplyReceiver = new BroadcastReceiver() { @RequiresApi(KITKAT_WATCH) @Override public void onReceive(final Context context, final Intent proxy_intent) {
		...
		try {
			...
			pending_intent.send(WeChatDecorator.this, 0, input_data, new PendingIntent.OnFinished() { @Override public void onSendFinished(final PendingIntent pendingIntent, final Intent intent, final int resultCode, final String resultData, final Bundle resultExtras) {
				...
				if (BuildConfig.DEBUG) Log.d(TAG, "Reply sent to " + intent.getStringExtra(KEY_CONVERSATION_ID) + ": " + text);
				final Bundle addition = new Bundle();
